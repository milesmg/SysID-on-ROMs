...
### ADJUSTED: Read the learner flag before choosing the FOM implementation file.
...
include(joinpath(REPO_ROOT, "Research_Code", "src", "HPC", "integration_AC_hpc.jl"))
### ADJUSTED: Select the NN or polynomial FOM helper from the same runner.
hpc_log("run_fom_hpc", "Loading NN FOM helper code")
include(joinpath(REPO_ROOT, "Research_Code", "src", "HPC", "FOM_opt_AC_hpc.jl"))
if learner == "polynomial"
    hpc_log("run_fom_hpc", "Loading polynomial FOM extension code")
...
### ADJUSTED: Parse spatial dimension for 1D/2D HPC FOM runs.
dimension = get_int(opts, "dimension", 1)
### ADJUSTED: Parse optional Allen-Cahn boundary-condition selection for FOM runs.
...
### ADJUSTED: Parse polynomial degree for polynomial FOM learning.
...
run_name = get_string(opts, "run-name", timestamp_run_name(learner == "polynomial" ? "FOM_AC_polynomial_hpc" : "FOM_AC_hpc"))
...
prepared = learner == "polynomial" ?
    prepare_for_optimization(; N, L, ε2, tspan=reference.tspan, N_obs, dimension, boundary_condition=reference.boundary_condition, u₀=reference.u₀, h, seed, polynomial_degree) :
    prepare_for_optimization(; N, L, ε2, tspan=reference.tspan, N_obs, dimension, boundary_condition=reference.boundary_condition, u₀=reference.u₀, h, seed)
run_params = merge(
    prepared.run_params,
    (;
        k,
        ### ADJUSTED: Save reference metadata needed to reconstruct 1D/2D trajectories.
        reference_parameters=reference.p₀,
        reference_saved_times=copy(reference.t),
        reference_algorithm=string(nameof(typeof(reference.u_ref.alg))),
        reference_dt=reference.Δt,
        reference_dt_factor,
        reference_save_count=length(reference.t),
        learner,
        polynomial_degree=learner == "polynomial" ? polynomial_degree : nothing,
