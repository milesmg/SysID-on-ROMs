...
### Integrate ROM ###
"""
Evaluate the Allen–Cahn POD/DEIM neural ROM right-hand side.
"""
...
function loss_ROM(prob, p, alg, sensalg)
    data = prob.f.f.data
    residual = model_ROM(prob, p, alg, sensalg) - data.u_ref_obs
    return 0.5 * data.Δx * sum(abs2, residual)
end
...
"""
Run an Adam ROM optimization.

- Args: `(optprob; η=5e-2, β=(0.9, 0.99), N_iter=400,
  warmup=true, save_frequency=nothing)`
...
function run_ROM_optimization(
    ...
    function callback(state, loss)
        ...
        if global_iteration % 10 == 0
            println("iteration = $(global_iteration), loss = $loss, last iteration = $(round(elapsed; digits=2)) s")
        end
