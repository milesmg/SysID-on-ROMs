...
if learner == "polynomial"
    hpc_log("run_fom_hpc", "Loading polynomial FOM extension")
    include(joinpath(REPO_ROOT, "Research_Code", "src", "HPC", "Simulations", "FOM_ROM_polynomial_learning_AC.jl"))
    prepared = prepare_for_optimization(; N, L, ε2, tspan=reference.tspan, N_obs, u₀=reference.u₀, h, seed, dimension=reference.dimension, boundary_condition=reference.boundary_condition, polynomial_degree)
elseif learner == "nn"
    prepared = prepare_for_optimization(; N, L, ε2, tspan=reference.tspan, N_obs, u₀=reference.u₀, h, seed, dimension=reference.dimension, boundary_condition=reference.boundary_condition)
else
