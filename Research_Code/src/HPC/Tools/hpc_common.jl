...
### ADJUSTED: Load shared guards from the moved Research_Code/src/Misc. directory.
...
- args: `N`, `L`, `ε2`, `k`, `tfinal`, `reference_dt_factor`, optional `dimension`, `boundary_condition`, and `u₀`
- returns: named tuple with `u_ref`, `prob`, `p₀`, `u₀`, `x`, `y`, `t`, `tspan`, `Δx`, `Δt`, `dimension`, `boundary_condition`, and `state_shape`; reference save times are capped at 500
"""
function build_ac_reference(; N, L, ε2, k, tfinal, reference_dt_factor, dimension=1, boundary_condition="homogeneous_dirichlet", u₀=nothing)
    ### ADJUSTED: Use the moved dimension/boundary-aware AC setup helpers.
    ...
    ### ADJUSTED: Cap saved reference times so ROM/FOM tooling does not build huge snapshot histories.
    save_count = min(max(2, floor(Int, (tspan[2] - tspan[1]) / Δt) + 1), 500)
    t = LinRange(tspan[1], tspan[2], save_count)
    ### ADJUSTED: Keep nonnumeric reference metadata out of ComponentVector trainable storage.
    p₀ = (; ε2, k, Δx, dimension, N, boundary_condition)
    jac = get_lap_ac_matrix(N, ε2, Δx, dimension, boundary_condition)
    f = ODEFunction(rhs_ac!; jac_prototype=jac)
