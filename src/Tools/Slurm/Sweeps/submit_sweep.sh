...
if [ "$#" -ne 1 ]; then
    echo "Usage: bash src/Tools/Slurm/Sweeps/submit_sweep.sh <sweep_file.txt>"
    exit 1
fi

REPO_ROOT="${REPO_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)}"
### ADJUSTED: Use the top-level Slurm and Data directories.
SWEEP_DIR="$REPO_ROOT/src/Tools/Slurm/Sweeps"
SLURM_LOG_DIR="$REPO_ROOT/Data/_Logs"
SWEEP_FILE="$1"
...
if [ "${DRY_RUN:-false}" = "true" ]; then
    ...
    echo "SWEEP_FILE=\"$SWEEP_FILE\" SWEEP_TOTAL_COUNT=\"$COUNT\" SWEEP_WORKER_COUNT=\"$WORKER_COUNT\" sbatch --array=\"$ARRAY_SPEC\" \"$SWEEP_DIR/run_sweep.slurm\""
    exit 0
fi

mkdir -p "$SLURM_LOG_DIR"
SWEEP_FILE="$SWEEP_FILE" SWEEP_TOTAL_COUNT="$COUNT" SWEEP_WORKER_COUNT="$WORKER_COUNT" sbatch --array="$ARRAY_SPEC" "$SWEEP_DIR/run_sweep.slurm"
