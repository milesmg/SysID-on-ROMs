...
### ADJUSTED: Keep only per-stage optimization and window schedules grouped.
GROUPED_KEYS = {"ETAS", "ITERS", "BETA", "WINDOW_T", "WINDOW_N_OBS", "WINDOW_START_POLICY"}
METADATA_KEYS = {"TARGET", "SWEEP_NAME", "RUN_NAME"}
CASE_SENTINEL = "__ROW_CASE__"


def canonical_key(key):
    ### ADJUSTED: Require one exact spelling for each sweep parameter.
...
def target_from_metadata(metadata):
    return metadata.get("TARGET", "").strip().lower()
...
def main(argv):
    parser = argparse.ArgumentParser(description="Expand FOM/ROM sweep files.")
