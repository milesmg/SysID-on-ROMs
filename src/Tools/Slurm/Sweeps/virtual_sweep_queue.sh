...
### ADJUSTED: Resolve source and queue state from the top-level layout.
REPO_ROOT="${REPO_ROOT:-$(cd "$SCRIPT_DIR/../../../.." && pwd)}"
SWEEP_DIR="$REPO_ROOT/src/Tools/Slurm/Sweeps"
LOG_DIR="$REPO_ROOT/src/Tools/Misc/VirtualQueue"
...
usage() {
    cat <<EOF
Usage:
  bash src/Tools/Slurm/Sweeps/virtual_sweep_queue.sh <sweep_file.txt> [more_sweeps...]
...
submit_queue_line() {
    ...
    ### ADJUSTED: Create the top-level Slurm output directory before submission.
    mkdir -p "$REPO_ROOT/Data/_Logs"
    ...
    if sbatch_output="$(
        ...
        sbatch --parsable --job-name "$VQ_JOB_NAME" --array="$combo_index-$combo_index" "$SWEEP_DIR/run_sweep.slurm"
    )"; then
