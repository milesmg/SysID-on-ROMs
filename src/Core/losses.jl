...
function weighted_solution_loss(model_states, reference_states, Δmeasure, normalization)
    total = sum(sum(abs2, model_state .- reference_state)
                for (model_state, reference_state) in zip(model_states, reference_states))
...
Args:
    ...
    - project indicates that the window has preprojected references for reduced-space comparison
...
function solve_window_loss(window::TrainingWindow, prob, p, alg, sensalg, normalization, Δmeasure, reconstruct, project, loss_space)
    ...
    if loss_space == "REDUCED" && !isnothing(project)
        weighted_solution_loss(sol.u, window.model_reference_observations, Δmeasure, normalization)
    else
